...
class AttendanceService {
  ...
  Future<AttendanceRecord?> getTodayRecord() async {
    ...
    try {
      ...
      print(
        '✅ Selected record: checkedIn=${selectedRecord.isCheckedIn}, store=${selectedRecord.storeName}',
      );
      print(
        '✅ Selected record times: checkIn=${selectedRecord.checkInTime}, checkOut=${selectedRecord.checkOutTime}',
      );
  ...
  Future<AttendanceRecord> checkIn({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
          '📋 Base record: ${record.id}, checkIn=${record.checkInTime}, checkOut=${record.checkOutTime}',
        );
        ...
        print(
          '💾 Saving updated record: checkIn=${updatedRecord.checkInTime}, checkOut=${updatedRecord.checkOutTime}, isCheckedIn=${updatedRecord.isCheckedIn}',
        );
        ...
        print(
          '🔍 Verification record: checkIn=${verifyRecord?.checkInTime}, checkOut=${verifyRecord?.checkOutTime}, isCheckedIn=${verifyRecord?.isCheckedIn}',
        );
      ...
      final record = await createTodayRecord();
      print(
        '📋 Fallback base record: ${record.id}, checkIn=${record.checkInTime}, checkOut=${record.checkOutTime}',
      );
      ...
      print(
        '💾 Fallback saving record: checkIn=${updatedRecord.checkInTime}, checkOut=${updatedRecord.checkOutTime}, isCheckedIn=${updatedRecord.isCheckedIn}',
      );
      await saveRecord(updatedRecord);
      ...
      final verifyRecord = await getTodayRecord();
      print(
        '🔍 Fallback verification: checkIn=${verifyRecord?.checkInTime}, checkOut=${verifyRecord?.checkOutTime}, isCheckedIn=${verifyRecord?.isCheckedIn}',
      );
    ...
    try {
      ...
      request.fields.addAll({
        'date':
            '${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')}',
        'store_id':
            currentRecord.storeId?.toString() ?? '0', // ✅ Add missing store_id
