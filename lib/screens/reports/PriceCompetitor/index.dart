...
class PriceCompetitorReportScreen extends StatefulWidget {
  ...
  @override
  State<PriceCompetitorReportScreen> createState() =>
      _PriceCompetitorReportScreenState();
}

class _PriceCompetitorReportScreenState
    extends State<PriceCompetitorReportScreen> {
  ...
  Future<void> _loadInitialData() async {
    await Future.wait([_loadStoreFromCheckIn(), _loadProducts()]);
  }
  ...
  Future<void> _loadStoreFromCheckIn() async {
    ...
    print('✅ Price Competitor: Using storeId from widget: ${widget.storeId}');
  }
  ...
  Future<void> _loadProducts() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'Error loading products: ${response.statusCode} - ${response.body}',
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        await ReportCompletionService.markReportCompleted(
          storeId: _storeId!,
          reportType: 'price_competitor',
          date: DateFormat('yyyy-MM-dd').format(_selectedDate!),
          completedAt: submissionTime,
          reportData: {
            'originId': 1, // Competitor
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Submit price information for competitor products',
              style: TextStyle(
                fontSize: 14,
                ...
                Text(
                  label.contains('Start')
                      ? (product.promoStartDate != null
                            ? DateFormat(
                                'dd MMM yyyy',
                              ).format(product.promoStartDate!)
                            : 'Select Start Date')
                      : (product.promoEndDate != null
                            ? DateFormat(
                                'dd MMM yyyy',
                              ).format(product.promoEndDate!)
                            : 'Select End Date'),
                  style: TextStyle(
                    fontSize: 14,
                    color:
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                ...
                color: value ? activeColor : (isPromo ? Colors.orange[900] : Colors.purple[900]),
                letterSpacing: 0.1,
              ),
            ),
            const SizedBox(height: 8),
            ...
            Row(
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                    fontSize: 14,
                    color: value ? activeColor : (isPromo ? Colors.orange[700] : Colors.purple[700]),
                    fontWeight: FontWeight.w500,
                  ),
                ),
              ],
...
class SubbrandData {
  ...
  factory SubbrandData.fromJson(Map<String, dynamic> json) {
    return SubbrandData(id: json['id'] ?? 0, name: json['name'] ?? '');
  }
}
