...
// Old StoreLocationScreen class removed - now using LocationUpdateScreen
  void dispose() {
    _reasonController.dispose();
    super.dispose();
  }
  ...
  Future<void> _updateStoreLocation() async {
    ...
    try {
      final response = await StoreMappingService.updateStoreLocation(
        ...
        latitudeNew: _currentPosition!.latitude,
        longitudeNew: _currentPosition!.longitude,
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text('ID: ${widget.store.id}'),
                  Text('Area: ${widget.store.areaName ?? 'Unknown'}'),
                  if (widget.store.address != null)
                    Text('Address: ${widget.store.address}'),
                ],
              ),
            ),
              ...
              child: Column(
                children: [
                  if (_currentPosition != null) ...[
                    ...
                    const SizedBox(height: 8),
                    Text(
                      'Distance: ${StoreMappingService.calculateDistance(
